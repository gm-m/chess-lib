...
import { ChessGame, SQUARE_TO_COORDS } from './chess-game';
...
describe.todo("Test getFullMoveNumber", () => {
    ...
    testCases.forEach(({ fen, expectedMoveNumber }) => {
        test(`getMoveNumber(${fen}) should return ${expectedMoveNumber}`, () => {
            ...
            expect(chessGame.getFullMoveNumber()).toEqual(expectedMoveNumber);
        });
    });
});

describe("Test getSideToMove", () => {
    const chessGame = new ChessGame();
    const testCases = [
        { fen: '4r1k1/b2N4/p1p1r2p/2Pp4/P4P2/2P3P1/7P/1R3R1K b - - 3 32', expectedOutput: 'b' },
        { fen: '8/8/8/2p1pR2/3rk1P1/2K5/8/8 w - - 6 67', expectedOutput: 'w' },
    ];

    testCases.forEach(({ fen, expectedOutput }, index) => {
        test(`getSideToMove for testCase with index: ${index}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.getSideToMove()).toEqual(expectedOutput);
        });
    });
});

describe("Test getFen", () => {
    const chessGame = new ChessGame();
    const testCases = [
...
describe("Test removePiece", () => {
    ...
    testCases.forEach(({ fen, square, expectedPiece }) => {
        test(`removePiece at square ${SQUARE_TO_COORDS[square]}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.removePiece(square)).toEqual(expectedPiece);
        });
    });
});

describe("Test getPieceSquares", () => {
    const chessGame = new ChessGame();
    const testCases = [
    ...
    testCases.forEach(({ fen, piece, expectedOutput }) => {
        test(`getPieceSquares(${fen}) should return ${expectedOutput}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.getPieceSquares(piece)).toEqual(expect.arrayContaining(expectedOutput));
        });
    });
});
...
describe("Test getSquare", () => {
    ...
    testCases.forEach(({ fen, square, expectedOutput }) => {
        test(`getSquare(${fen}) should return ${expectedOutput}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.getSquare(square)).toEqual(expectedOutput);
        });
    });
});
...
describe("Test getMaterialAdvantage", () => {
    ...
    testCases.forEach(({ fen, expectedScore }) => {
        test(`getScore(${fen}) should return ${expectedScore}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.getMaterialAdvantage()).toEqual(expectedScore);
        });
    });
});
...
describe("Test getBoardPieces", () => {
    ...
    testCases.forEach(({ fen, expectedPieces }) => {
        test(`getSquare(${fen}) should return ${expectedPieces}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.getBoardPieces()).toEqual(expectedPieces);
        });
    });
});
...
describe("Test isLegalMove", () => {
    ...
    const testCases = [
        ...
        {
            fen: '2krb3/b6r/p1p1Q3/4pp1N/2P4p/1R6/P3B1PP/7K b - - 0 27',
            move: { fromSquare: Square.d8, toSquare: Square.d7 },
            expectedOutput: true
        },
        {
            fen: '2krb3/b6r/p1p1Q3/4pp1N/2P4p/1R6/P3B1PP/7K b - - 0 27',
    ...
    testCases.forEach(({ fen, move, expectedOutput }) => {
        test(`isLegalMove(${move.fromSquare} -> ${move.toSquare}) should return ${expectedOutput}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.isLegalMove(move)).toBe(expectedOutput);
        });
    });
});
...
describe.skip("Test filterIllegalMoves", () => {
    ...
    testCases.forEach(({ fen, expectedOutput }) => {
        test(`filterPseudoLegalMoves should return ${expectedOutput}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.filterPseudoLegalMoves()).toEqual(expectedOutput);
        });
    });
});
...
describe("Test isSquareAttacked", () => {
    ...
    testCases.forEach(({ fen, square, attackedFromColor, expectedOutput }) => {
        test(`isSquareAttacked(${square}) should return ${expectedOutput}`, () => {
            chessGame.loadFen(fen);
            const evaluator = new GameStateEvaluator(chessGame.boardState);
            expect(evaluator.isSquareAttacked(square, attackedFromColor)).toBe(expectedOutput);
        });
    });
});
...
describe("Test isInCheck", () => {
    ...
    testCases.forEach(({ fen, side, expectedOutput }) => {
        test(`isInCheck(${side}) should return ${expectedOutput}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.isInCheck(side)).toBe(expectedOutput);
        });
    });
});
...
describe("Test isCheckmate", () => {
    ...
    testCases.forEach(({ fen, expectedCheckmate }) => {
        test(`isCheckmate(${fen}) should return ${expectedCheckmate}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.isCheckmate()).toBe(expectedCheckmate);
        });
    });
});
...
describe("Test isInsufficientMaterial", () => {
    ...
    testCases.forEach(({ fen, expectedOutput }) => {
        test(`isInsufficientMaterial(${fen}) should return ${expectedOutput}`, () => {
            chessGame.loadFen(fen);
            expect(chessGame.isInsufficientMaterial()).toBe(expectedOutput);
        });
    });
});
